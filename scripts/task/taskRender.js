...
function subTaskListRender() {
  ...
  element.innerHTML = "";
  element.innerHTML += taskSubTaskList
    .map((designation, index) => SubtaskListTemplate(designation, index))
    .join("");
}
...
function taskRenderContactList(index, name, color, email, check) {
  ...
  element.innerHTML += taskContacListTemplate(
    index,
    name,
    contactColorAssign(color),
